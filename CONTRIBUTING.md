...
Run the required regression suite matching CI:

```bash
./tests/test_cli_dispatch.sh
```

You are also highly encouraged to run these local syntax and format checks:
...
If a required tool is unavailable, report that explicitly in the pull request instead of claiming the check passed.
